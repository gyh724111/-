...
import com.gyh.resumeapp.Utils.ThreadUtil.ThreadPram;
...
public class WebUtil {
    ...
    public static JSONObject GetJSONObject(String jsonUrl) {
        String webData = WebUtil.GetString(jsonUrl);
    ...
    public static JSONArray GetJSONArray(String jsonUrl) {
        String webData = WebUtil.GetString(jsonUrl);
        JSONArray webJson = null;
        try {
            webJson = new JSONArray(webData);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return webJson;
    ...
    public static byte[] GetBytes(final String url) {
        ...
        // 在非主线程中执行网络请求，获取数据
        ThreadUtil.RunInCachedThread(new ThreadPram() {
            @Override
            public void Function() {
                byte[] data = GetBytes_process(url);
                GetBytesDic.put(KEY, data);
                Log.e("webutil","data:"+data.toString());
            }
        });

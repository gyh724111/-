...
public class ResumeCertAdapter extends ListViewCommonAdapterWithoutdiv<ResumeCertDetail> {
    /**
     ...
     */
    public ResumeCertAdapter(Context context, String listIteam_LayoutName, ResumeCertDetail[] IteamDatas) {
     ...
     */
    @Override
    public void setIteamView(ResumeCertDetail iteamData) {
        // TODO 根据列表项数据iteamData，设置列表项的显示

        TextView tv_cert = (TextView) IteamView("resume_cert_text");
        if (iteamData.cert != ""){
            tv_cert.setText(iteamData.cert);
        }
     ...
     */
    @Override
    public void setIteamClick(Context iteamContext, ResumeCertDetail iteamData) {
        // TODO 根据列表项数据iteamData，设置列表项的点击处理逻辑
        // View view = IteamView("name"); //获取列表项中，指定名称的view
        Log.e("ResumeCertAdapter","点击列表的id:"+iteamData.id);
